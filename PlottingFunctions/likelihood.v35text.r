...
extractSMRR<-function(scenNum,runDir, modelResultID) { 
  ...
  # if predictions for expected numbers of severe events are available use these.  Otherwise use the number of events
  pred$severe<- preferredOutput(output, 1, 1, modelResultID$expectedSevere, modelResultID$nSevere)
...
extractMort <-function(scenNum,runDir, modelResultID) { 
  ...
  pred$dirDeaths<- preferredOutput(output, 1, 1, modelResultID$expectedDirectDeaths, modelResultID$nDirDeaths)
...
#------------------------------------------------------------------------------
OBJ_DirectMalriaMortality <- function(fieldDataAll, fieldDataID, modelResultID, runDir){
...
allPlots<-function(runID,dirname,params){
  runDir <- paste(sep = "","./PriorDraws/Output/checks/")
  ...
  fieldDataAll <-read.table(paste("./PriorDraws/LikelihoodComp/fieldData",".txt", sep=""))
  ...
  LF$obj_9 <- OBJ_DirectMalriaMortality(fieldDataAll, fieldDataID, modelResultID, runDir)
