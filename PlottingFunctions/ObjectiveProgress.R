...
#
############################################################
pdf("./OutputPlots/Fig_2_A_covergence_plot.pdf" , width= 5.07, height = 4.11)
ggplot(data=time_frame_all, aes(x=elapsed, y=best_known, group=Algorithm,color=Algorithm)) + 
  ...
  #previous
  geom_hline(yintercept=73, linetype="dashed", color="grey40", size=1) +
  geom_text(aes(max(time_frame_all$elapsed),73,label = 'satisfactory',vjust=1.3, hjust = "inward"), color="grey40")+
  geom_hline(yintercept=63.13401, linetype="dashed", color="grey40", size=1) + 
  geom_text(aes(max(time_frame_all$elapsed),63.13401,label = "current fit",vjust=1.3, hjust = "inward"), color="grey40")+
  #axes:
  ...
  labs(x = 'CPU time (seconds)', y = TeX("Current best LF")) +
  theme_classic() + 
