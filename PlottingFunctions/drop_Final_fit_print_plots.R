#finalfit_drop_point
GitDir =  "/scicore/home/pothin/reiker/GitRepos/om_fitting/"
MainExperimentDir = "/scicore/home/pothin/reiker/Paper_3_Model_Fitting/2020_05_full_fit_small_hetGP_norm_sampling/"
old_param_output_dir = "/scicore/home/pothin/reiker/Paper_3_Model_Fitting/2020_04_full_fit_small_hetGP/PriorDraws/Output/old_params/"
GP_Dir = "/scicore/home/pothin/reiker/Paper_3_Model_Fitting/2020_05_full_fit_small_hetGP_norm_sampling/"
GPSG_Dir = "/scicore/home/pothin/reiker/Paper_3_Model_Fitting/2020_05_full_fit_small_GPSG_norm_sampling/"
GPDrop_Dir = "/scicore/home/pothin/reiker/Paper_3_Model_Fitting/2020_10_drop_point/"
...
# =============================================================================
# best_hetgp_drop_params
# =============================================================================

setwd(paste0(GPDrop_Dir))

load(paste0(GPDrop_Dir,'time.hetgp'))
...
time_frame_hetGP_drop$best_known[c(1:length(hetgp_best))]=hetgp_drop_best
...
for(i in 2:dim(time_frame_hetGP_drop)[1]){
  if(time_frame_hetGP_drop$elapsed[i]<time_frame_hetGP_drop$elapsed[i-1]){
    ...
    for(j in (dim(time_frame_hetGP_drop)[1]-1):i){time_frame_hetGP_drop$best_known[j+1] = time_frame_hetGP_drop$best_known[j]}
    time_frame_hetGP_drop$best_known[i] = time_frame_hetGP_drop$best_known[i-1]
...
best_gp_drop_iter =time_frame_hetGP_drop[which.min(time_frame_hetGP_drop$best_known),"iteration"] 
gp_drop_theta = evaluations$best[[best_gp_drop_iter]]$known$parameters
gp_drop_params = quant.to.prior(gp_drop_theta)
...
GPDroprunID <- paste0("output_main_",sprintf("%04i",best_gp_drop_iter))
GPDroprunDir <- paste0(GPDrop_Dir,"PriorDraws/Output/small/",GPDroprunID ,"/")


GPSGrunID <- paste0("output_main_",sprintf("%04i",best_gpsg_iter))
GPSGrunDir <- paste0(GPSG_Dir,"PriorDraws/Output/small/",GPSGrunID,"/")

OldrunDir <- paste0("/scicore/home/pothin/reiker/Paper_3_Model_Fitting/2020_04_full_fit_small_hetGP/PriorDraws/Output/old_params/")
...
densBiasGPDrop$nonGarki <- gp_drop_params$DensityBiasnonGarki
densBiasGPDrop$garki <- gp_drop_params$DensityBiasGarki
...
# --------------------------------------------------------------------
# set loss Vector etc
lossVector <- array(NA,dim=c(11,4))
lossVector_logLH <- array(NA,dim=c(11,4))
LF <- list()
LF$GP <- list()
LF$GPDrop <- list()
LF$GPSG <- list()
...
LF$GPDrop$obj_1 <- OBJ_AgePatternIncidenceAfterIntervention(fieldDataAll, fieldDataID, modelResultID, densBiasGPDrop, GPDroprunDir)
...
pdf(file="./OutputPlots/Drop_Model_fit_Objective_1.pdf",width=6.98,height=5.65)
ggplot(Obj1_gathered,aes(x=age,y=value))+
  ...
  scale_linetype_manual(name="Parameterization",values = c(gp="solid",gpsg="solid",gpdrop="solid",old="dashed"), labels = c(gp="GP-BO", gpdrop="GP-BO (drop)", gpsg="GPSG-BO", old="GA-O")) + 
  scale_colour_manual(name="Parameterization",values=c(gp=colours[2],gpdrop=colours[3],gpsg=colours[4],old=colours[1]), labels = c(gp="GP-BO", gpdrop="GP-BO (drop)",gpsg= "GPSG-BO",old= "GA-O")) +
...
lossVector[1,3] <- LF$GPDrop$obj_1$LF$orig$total
lossVector_logLH[1,3] <-  LF$GPDrop$obj_1$LF$logLH$total

lossVector[1,4] <- LF$GPSG$obj_1$LF$orig$total
lossVector_logLH[1,4] <-  LF$GPSG$obj_1$LF$logLH$total
...
LF_temp$GPDrop <- OBJ_AgePatternPrevalenceAndDensity(fieldDataAll, fieldDataID, modelResultID, densBiasGPDrop, GPDroprunDir)
...
for(i in 1: length(LF_temp$GP$Data$FieldData)){
  ...
  Obj2_ls[[i]]$gpdrop = LF_temp$GPDrop$Data$ModelResults[[i]]$prevalence
...
pdf(file="./OutputPlots/Drop_Model_fit_Objective_2.pdf",height=5.13,width=7.76)
...
for(i in 1: length(LF_temp$GP$Data$FieldData)){
  ...
  Obj3_ls[[i]]$gpdrop = LF_temp$GPDrop$Data$ModelResults[[i]]$density
...
pdf(file="./OutputPlots/Drop_Model_fit_Objective_3.pdf",height=5.82,width=9.32)
...
LF$GPDrop$obj_4 <- OBJ_AgePatternMOI(fieldDataAll, fieldDataID, modelResultID, GPDroprunDir)
...
Obj4$gpdrop = LF$GPDrop$obj_4$Data$ModelResults$moi
...
pdf(file="./OutputPlots/Drop_Model_fit_Objective_4.pdf",height=4.24,width=4.98)
ggplot(Obj4_gathered,aes(x=age,y=value))+
  ...
  theme_light() + 
  theme(legend.position = c(0.99,0.80),
        legend.justification = "right",
...
LF$GPDrop$obj_5a <- OBJ_AgePatternIncidenceClinicalMalaria_Senegal(fieldDataAll, fieldDataID, modelResultID, GPDroprunDir)
...
Obj5a_ls$Ndiop$gpdrop = LF$GPDrop$obj_5a$Data$ModelResults$Ndiop$incid
...
Obj5a_ls$Dielmo$gpdrop = LF$GPDrop$obj_5a$Data$ModelResults$Dielmo$incid
...
pdf(file="./OutputPlots/Drop_Model_fit_Objective_5a.pdf",height=3.90,width=6.34)
...
LF$GPDrop$obj_5b <- OBJ_AgePatternIncidenceClinicalMalaria_Idete(fieldDataAll, fieldDataID, modelResultID, GPDroprunDir)
...
Obj5b$gpdrop = LF$GPDrop$obj_5b$Data$ModelResults$Idete$incid
...
pdf(file="./OutputPlots/Drop_Model_fit_Objective_5b.pdf",height=3.86,width=6.34)
...
LF$GPDrop$obj_6 <- OBJ_AgePatternThresholdClinicalAttack(fieldDataAll, fieldDataID, modelResultID, densBiasGPDrop, GPDroprunDir)
...
Obj6$gpdrop = LF$GPDrop$obj_6$Data$ModelResults$NdiopDielmo$pyrogt
...
pdf(file="./OutputPlots/Drop_Model_fit_Objective_6.pdf",height=3.41,width=5.38)
...
LF$GPDrop$obj_7 <- OBJ_SevereEpisodesVsPrevalence(fieldDataAll, fieldDataID, modelResultID,  GPDroprunDir)
...
Obj7$gpdrop = data.frame(prev= LF$GPDrop$obj_7$Data$ModelResults$prevalence)
Obj7$gpdrop$episodes = LF$GPDrop$obj_7$Data$ModelResults$sevEpPer1000personYr
Obj7$gpdrop$source = "gpdrop"
...
pdf(file="./OutputPlots/Drop_Model_fit_Objective_7.pdf",width=6.49,height=5.17)
ggplot(Obj_7_gathered,aes(x=prev,y=episodes))+
  ...
  scale_shape_manual(name="Parameterization",values=c(data = 16,gp=15,gpdrop=18,gpsg=17,old=3), labels = c(data="Data",gp="GP-BO",gpdrop="GP-BO (drop)",gpsg= "GPSG-BO",old= "GA-O")) +
  scale_colour_manual(name="Parameterization",values=c(gp=colours[2],gpdrop=colours[3],gpsg=colours[4],old=colours[1]), labels = c(gp="GP-BO", gpdrop="GP-BO (drop)",gpsg= "GPSG-BO",old= "GA-O")) +
  ...
  theme_light() + 
  theme(legend.position = c(0.99,0.82),
        legend.justification = "right",
        legend.title = element_text(face="bold",size = 12),
        legend.text = element_text(size = 10),
...
LF$GPDrop$obj_8 <- OBJ_AgePatternOfSevere(fieldDataAll, fieldDataID, modelResultID,  GPDroprunDir)
...
Obj8_ls$Sukuta$gpdrop = LF$GPDrop$obj_8$Data$ModelResults$Sukuta$sevEpPer1000personYr[1:3]
...
Obj8_ls$KilifiNorth$gpdrop = LF$GPDrop$obj_8$Data$ModelResults[["Kilifi-north"]]$sevEpPer1000personYr[1:3]
...
Obj8_ls$KilifiSouth$gpdrop = LF$GPDrop$obj_8$Data$ModelResults[["Kilifi-north"]]$sevEpPer1000personYr[1:3]
...
Obj8_ls$Siaya$gpdrop = LF$GPDrop$obj_8$Data$ModelResults$Siaya$sevEpPer1000personYr[1:3]
...
pdf(file="./OutputPlots/Drop_Model_fit_Objective_8.pdf",height=5.06,width=7.50)
...
LF$GPDrop$obj_9 <- OBJ_DirectMalariaMortality(fieldDataAll, fieldDataID, modelResultID,  GPDroprunDir)
...
Obj9$gpdrop = NA
for(i in 1:dim( Obj9)[1]){ Obj9$gpdrop[i] = LF$GPDrop$obj_9$Data$ModelResults[[i]]$dirDeathsPer1000personYr}
Obj9$gpsg = NA
...
pdf(file="./OutputPlots/Drop_Model_fit_Objective_9.pdf",height=4.04,width=5.44)
...
LF$GPDrop$obj_10 <- OBJ_indirectMortality(fieldDataAll, fieldDataID, modelResultID,  GPDroprunDir)
...
Obj10$gpdrop = NA
for(i in 1:dim( Obj10)[1]){ Obj10$gpdrop[i] = LF$GPDrop$obj_10$Data$ModelResults[[i]]$allCauseIMR}
Obj10$gpsg = NA
...
pdf(file="./OutputPlots/Drop_Model_fit_Objective_10.pdf",height=4.04,width=5.44)
