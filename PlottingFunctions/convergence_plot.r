...
GitDir = "/scicore/home/smith/pothin/GitRepos/om_fitting/" # Local version of the Git repository
...
pdf("./OutputPlots/Fig_2_A_covergence_plot_reviewed.pdf" , width= 5.07, height = 4.11)
ggplot(data=time_frame_all, aes(x=elapsed, y=best_known, group=Algorithm,color=Algorithm)) + 
  ...
  #colors:
  scale_colour_manual(values=colours, labels=c("GP-BO", "GPSG-BO")) +
  scale_fill_manual(values=colours, labels=c("GP-BO", "GPSG-BO")) +
  scale_shape_manual(values=c(16,17),labels=c("GP-BO", "GPSG-BO")) +
  ...
  labs(x = 'time (seconds)', y = TeX("Current best (min $F(\\theta))$")) +
  theme_classic() + 
